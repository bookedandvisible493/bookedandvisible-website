/*
 * Cookie consent banner for pages that load Google Analytics (gtag.js).
 * Works with the Google Consent Mode v2 defaults set inline in <head>
 * (analytics_storage etc. start denied). This script:
 *   1. On return visits, replays the visitor's stored choice via
 *      gtag('consent','update', ...) so Analytics keeps respecting it.
